...
@app.route('/records2', methods=['POST']) 
def create_a_record():
...
@app.route('/records2/<bandname>', methods=['POST'])
def create_a_album(bandname):
        if not request.json or not 'album' in request.json:
                return jsonify({'error':'the new album needs to have a name'}), 400
        new_album = {
                'album': request.json['album'],
                'songs': request.json.get('song', '')
        }
        all_records.append(new_album)
        return jsonify({'message': 'created: /records/<bandname>{}'.format(new_album['album'])}), 201
